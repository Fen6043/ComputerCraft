...
function RemoveTrash()
    local trashMaterials = {["minecraft:cobblestone"] = true,["minecraft:blackstone"] = true,["minecraft:cobbled_deepslate"] = true,["minecraft:gravel"] = true,["minecraft:tuff"] = true}
    for i = 1, 16, 1 do
        if turtle.getItemDetail(i) then
            if trashMaterials[turtle.getItemDetail(i).name] then
                turtle.select(i)
                turtle.dropDown()
            end
        end
...
function Mine()
    ...
    if input == "y" then
        ...
        if not turtle.getItemDetail(1) then
            print("Keep slab material in slot 1")
            return
        end
        slabMaterial = turtle.getItemDetail(1).name
    end

    if not CTL.CheckFuel((x * y * z) + (2 * depth) + (x + y)) then
        return
    end
    ...
    for j = 0, z, 1 do
        for i = 0, x, 1 do
            if ydirection == 0 and xdirection == 1 then
                CTL.MovetoLocation(i,y,CTL.cz,putSlab,slabMaterial)
            elseif ydirection == 2 and xdirection == 1 then
                CTL.MovetoLocation(i,0,CTL.cz,putSlab,slabMaterial)
            elseif ydirection == 0 and xdirection == 3 then
                CTL.MovetoLocation(x-i,y,CTL.cz,putSlab,slabMaterial)
            else
                CTL.MovetoLocation(x-i,0,CTL.cz,putSlab,slabMaterial)
            end
